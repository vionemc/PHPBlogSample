...
$isi = str_replace("'", "''", $isi);
$waktu = $_GET['waktu'];
$sql = "INSERT INTO komentar (id_post,username,isi_komentar,waktu_comment) VALUES('$idpost','$username','$isi','$waktu')";
$result = mysql_query($sql);
if (!$result) {
    echo $sql . mysql_error();
} else {
//update achievement komen
    $sql = "SELECT achievement_comment FROM profil WHERE username='" . $username . "'";
    $result = mysql_query($sql);
    $row = mysql_fetch_array($result);
    $achieve = $row['achievement_comment'] + 1;
    $sql = "UPDATE profil SET achievement_comment='" . $achieve . "' WHERE username='" . $username . "'";
    $result = mysql_query($sql);
    if (!$result) {
